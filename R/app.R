...
initializing <- function() {
  ...
  if(is.null(spill)) {
    ...
    colnames(spill) <- bc$fluoCh
  }
  bc$compDFs <- list(spill)
  ...
  bc$completeFluoCh <- completeFluoCh[!grepl("FS", completeFluoCh)]
  bc$completeFluoCh <- bc$completeFluoCh[!grepl("SS", bc$completeFluoCh)]
...
server <- function(input, output, session) {
      ...
      if(length(bc$tempFileList) == 0) {
        ...
        if(all(verifier)) {
          ...
          if(length(unique(channelLengths)) == 1) {
            ...
            if(is.null(unlist(tubeNameList))) {
              tubeNameList <- substr(bc$tempFileList, 1,
                                     nchar(bc$tempFileList) - 4)
            }
            ...
            if(all(verifier)) {
              ...
              if(is.null(unlist(tubeList))) {
                tubeList <- substr(bc$tempFileList, 1,
                                   nchar(bc$tempFileList) - 4)
              }
  ...
  observeEvent(reactBCytoFile$d, {
    ...
    tempList <- sapply(list.files(bc$filePath), function(x)
      strsplit(x, "")[[1]])
    IDs <- vapply(tempList, function(x)
      all(x[(length(x)-3):length(x)] == c(".","f","c","s")), TRUE)
    verifier <- identical(tempEnv$fileList, list.files(bc$filePath)[IDs])
    if(all(verifier)) {
      bc$loadingFile <- TRUE
      withProgress(message="Please wait...", detail="", value=0,
                   max=100, {
                     ...
                     preFluoCh <- colnames(bc$gs)[!grepl("FS",
                                                         colnames(bc$gs))]
                     ...
                     bc$completeFluoCh <- completeFluoCh[
                       !grepl("FS", completeFluoCh)]
                     bc$completeFluoCh <- bc$completeFluoCh[
                       !grepl("SS",bc$completeFluoCh)]
